...
public class PlayerMove : NetworkBehaviour
{
    ...
    [Header("말풍선 연결")]
    [SerializeField] private GameObject bubbleBackground; // 말풍선 전체 오브젝트
    [SerializeField] private TMP_Text bubbleText;         // 텍스트 오브젝트

    public void DisplaySpeechBubble(string message)
    {
        Debug.Log("DisplaySpeechBubble 함수가 실행되었습니다! 메시지: " + message); // 👈 이게 찍히는지가 핵심!
        ...
        if (bubbleBackground != null && bubbleText != null)
        {
            ...
            Debug.Log("말풍선 오브젝트가 활성화되었습니다."); // 👈 이것도 찍히는지 확인!
        ...
        {
            Debug.LogError("연결이 안 되어 있습니다! 인스펙터를 확인하세요.");
        }
    }
    ...
    private IEnumerator HideBubbleRoutine(GameObject bubbleObj, float delay)
    {
        ...
        bubbleObj.SetActive(false); // 3초 뒤 말풍선 숨김
    }
}